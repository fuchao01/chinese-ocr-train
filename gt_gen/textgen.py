...
def read_text_split(p=None, length=10, lineLength=300):
    """
    获取语料文本数据
    按照10个字一行分隔
    """
...
def rand_draw(angleTuple=(-10, 10), texts=None, length=10, back=True):
    """
...
def write_img_text(im, text, root='data/0'):
    ...
    if len(text) <= maxLen and len(text) >= minLen or maxLen is None:
        ...
        if len(im.split())==4:
            r,g,b,a=im.split()        #利用split和merge将通道从四个转换为三个
            im=Image.merge("RGB",(r,g,b))
            im.save(raw_imgPath)
        else:
            im.save(raw_imgPath)
        gt_count += 1
        if len(im_erase.split())==4:
            r, g, b, a = im_erase.split()
            im_erase = Image.merge("RGB", (r, g, b))
...
backPaths = glob.glob('/data1/sina_recmd/fuchao/chinese-ocr-train/gt_gen/bg_img/*.png')  ##背景图像
fonts = glob.glob('/data1/sina_recmd/fuchao/chinese-ocr-train/gt_gen/fonts/*.*')  ##字体集
corpusPaths = glob.glob('/data1/sina_recmd/fuchao/chinese-ocr-train/gt_gen/corpus/sougou_news/*.txt')  ##语料库
maxLen = 6  ##每行字符个数
minLen = 4
gt_count = 0
if __name__ == '__main__':
    ...
    for i in range(8000):
        ...
        get_img_text(angle=(-2, 2),
                     root='/data1/sina_recmd/fuchao/chinese-ocr-train/data',
                     back=True)
    # maxLen =   ##每行字符个数
    # for i in range(8000):
    #     print(i)
